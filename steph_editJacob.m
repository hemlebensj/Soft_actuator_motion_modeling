...
% find the midpoints (average in each direction)
midpt1 = [ mean([X_away_top,X_away_ridge],2) mean([Y_away_top,Y_away_ridge],2) ...
    mean([Z_away_top,Z_away_ridge],2) ];
midpt2 = [ mean([X_close_top,X_close_ridge],2) mean([Y_close_top,Y_close_ridge],2) ...
    mean([Z_close_top,Z_close_ridge],2) ];

% calculate the distance between midpoints (euclidean)
ptdist = sqrt( (midpt1(:,1)-midpt2(:,1)).^2 + (midpt1(:,2)-midpt2(:,2)).^2 + ...
    (midpt1(:,3)-midpt2(:,3)).^2 );

% find elongations by normalizing distance with initial condition
elongs = (ptdist-ptdist(1))./ptdist(1);
