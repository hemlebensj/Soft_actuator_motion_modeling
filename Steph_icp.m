...
X = reshape(X_away_top,1,[]);
Y= reshape(Y_away_top,1,[]);
Z = reshape(Z_away_top,1,[]); 
% X = reshape(X,1,[]);
% Y = reshape(Y,1,[]);
...
title('Red: Top actuator points away, blue: transformed point cloud');
...
axis equal;
xlabel('x'); ylabel('y'); zlabel('z');
title('Red: Top away points on actuator, blue: transformed point cloud');
